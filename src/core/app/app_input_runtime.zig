...
test "app_input_runtime routes auth picker navigation before composer history" {
    ...
    // omfx: the second root action is the OpenAI sign-in.
    try std.testing.expect((auth_runtime.Choice{ .action = .login_openai }).eql(app.auth.pickerView().selected_choice.?));
...
fn openRoutingAuthPicker(app: *RoutingFakeApp) !void {
    ...
    // omfx: the root hub also offers the direct provider sign-ins.
